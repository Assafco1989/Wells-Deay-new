...
def _message_card(status:str, ts:str, top_feature:str, rmse:float, per_thr:float, global_thr:float, k_thr:float, action:str) -> str:
    ...
    <div style="
        ...
        ">
      <div style="font-weight:700; font-size:19px; margin-bottom:6px;">{title}</div>
	...
	<b>Feature RMSE:</b> {rmse:.4f}<br/>
        ...
        <b>Global threshold:</b> {global_thr:.4f}
...
# -----------------------
# Helper: infer well code from training CSV filename (A1,A2,A3,K1..K6)
# Uses lookarounds so it matches inside names like "Well_A1_train.csv"
# -----------------------
_WELL_RE = re.compile(r'(?<![A-Za-z0-9])(A1|A2|A3|K1|K2|K3|K4|K5|K6)(?![A-Za-z0-9])', re.IGNORECASE)

def _infer_well_from_filename(name: str):
    if not name:
        return None
...
if "STATE" not in st.session_state:
    st.session_state.STATE = {
        ...
        "train_file_name": "",   # <-- remember uploaded train CSV filename
    }
...
with tabs[0]:
    ...
    with cL:
        ...
        if train_file is not None:
            try:
                S["train_file_name"] = getattr(train_file, "name", "")
                S["train_df"] = pd.read_csv(train_file)
                S["train_df"] = standardize_well_df(S["train_df"], add_only=True)
                st.success(f"Training CSV loaded{(' — ' + S['train_file_name']) if S['train_file_name'] else ''}.")
    ...
    # Render config controls ONLY after a training CSV is uploaded.
    # This removes the transient error seen before upload.
    if S["train_df"] is None:
        st.info("Upload a training CSV to configure feature columns and sampling.")
        # Create harmless defaults so later tabs don't KeyError
        st.session_state.setdefault("ts_col", TIME_COL_STD)
        st.session_state.setdefault("feat_cols", [])
        st.session_state.setdefault("samp_secs", 3600)
        st.session_state.setdefault("win_min", 360)
        st.session_state.setdefault("stride_min", 60)
        st.session_state.setdefault("topfeat_k", 0.35)
        FEATURE_COLS = []
        TIMESTAMP_COL = TIME_COL_STD
    else:
        TIMESTAMP_COL = st.selectbox(
            "Timestamp column",
            options=[TIME_COL_STD] + list(S["train_df"].columns),
            index=0,
            key="ts_col"
        )

        # Safe defaults for multiselect
        available_cols = list(S["train_df"].columns)
        suggested = [c for c in FEATURE_COLS_STD if c in available_cols]         # canonical
        fallback  = [c for c in ["Column2","Column4","Column6"] if c in available_cols]
        default_feats = suggested if suggested else fallback
        safe_default  = [c for c in default_feats if c in available_cols]

        FEATURE_COLS = st.multiselect(
            "Feature columns (e.g., Current, Flow, Pressure)",
        ...
        # User control for top-feature factor K
        TOPFEAT_K = st.slider("Top-feature factor K (× global thr)", min_value=0.05, max_value=2.0, value=0.35, step=0.05, key="topfeat_k")

    # Readiness check (without throwing before upload)
    ready = (
        (S["train_df"] is not None) and
        (S["live_df"]  is not None) and
        (len(st.session_state.get("feat_cols", [])) > 0) and
        (st.session_state.get("ts_col", TIME_COL_STD) in (list(S["train_df"].columns) if S["train_df"] is not None else []))
    )
    ...
    if not S["train_ready"]:
        ...
        if start:
            try:
                ...
                Z = zapply(df_train_p[st.session_state["feat_cols"]], scaler, st.session_state["zclip"]).to_numpy()
                ...
                X = np.empty((0,1,len(st.session_state.get("feat_cols", []))))
            ...
            if X.shape[0] == 0:
                ...
                m = LSTMAE(n=X.shape[2], latent=int(st.session_state["latent"]), h=(int(st.session_state["h1"]), int(st.session_state["h2"]))).to(device)
                opt = torch.optim.Adam(m.parameters(), lr=float(st.session_state["lr"]))
                lossf = nn.MSELoss()
                ...
                msg = st.empty()
                for ep in range(int(st.session_state["epochs"])):  # EPOCHS
                    m.train()
                    opt.zero_grad()
                    y = m(Xt)
                    loss = lossf(y, Xt)
                    ...
                    prog.progress(int((ep+1)/st.session_state["epochs"]*100), text=f"Training... {ep+1}/{st.session_state['epochs']}  loss={loss.item():.6f}")
                    if (ep+1) % max(1, int(st.session_state["epochs"])//5) == 0:
                        msg.write(f"epoch {ep+1}/{st.session_state['epochs']}  loss={loss.item():.6f}")
...
with tabs[2]:
    ...
    # Title based on training CSV filename
    well_code = _infer_well_from_filename(S.get("train_file_name", ""))
if well_code:
    WELL_TITLE_COLOR = "#006edc"  # <- change this hex to any color you want
    st.markdown(
        f"""
        <div style="font-size:1.4rem; font-weight:700; color:{WELL_TITLE_COLOR}; margin:0.25rem 0 0.6rem 0;">
            Water Well {well_code}
        </div>
        """,
    ...
    if S["model_state"] is None or S["scaler"] is None:
        ...
        try:
            ...
            df_live_p = resample_and_fill(
                df_live[[st.session_state["ts_col"]] + st.session_state["feat_cols"]],
                st.session_state["ts_col"],
                st.session_state["samp_secs"],
                st.session_state["feat_cols"]
            )
            # Safe default for zclip in case Train tab wasn't opened yet
            zclip_val = st.session_state.get("zclip", 5.0)
            Z_live = zapply(df_live_p[st.session_state["feat_cols"]], S["scaler"], zclip_val).to_numpy()
